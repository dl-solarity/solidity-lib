...
abstract contract MultiOwnable is IMultiOwnable, Initializable {  
    ...
    */    
    ...
    function renounceOwnership() external virtual override onlyOwner {
        require(_owners.length() > 1, "MultiOwnable: only one owner, cannot renounce ownership");
    ...
    */
    function _checkOwner() internal view virtual {
        require(_owners.contains(msg.sender), "MultiOwnable: caller is not the owner");
    }
    ...
    function _addOwners(address[] memory newOwners) internal virtual {
        for (uint i = 0; i < newOwners.length; i++) {
            require (newOwners[i] != address(0), "MultiOwnable: zero address can not be added");
    ...
    function _removeOwners(address[] memory oldOwners) internal virtual {
        ...
        require(_owners.length() >= 1, "MultiOwnable: no owners left after removal");
