...
describe("Schnorr256", () => {
  ...
  const schnorrKeyPair = () => {
    const privKey = bytesToNumberBE(secp256k1.utils.randomSecretKey());
  ...
  const schnorrSign = function (hashedMessage: string, privKey: bigint, pubKey: AffinePoint<bigint>) {
    ...
    const e = (k + c * privKey) % secp256k1.Point.CURVE().n;
  ...
  const schnorrAdaptorSign = function (
    ...
    const RT = secp256k1.Point.fromAffine(R).add(secp256k1.Point.fromAffine(T)).toAffine();
  ...
  describe("adaptorVerify", () => {
    it("should verify the adaptor signature", async () => {
      ...
      const T = secp256k1.Point.BASE.multiply(t).toAffine();
    ...
    it("should not verify if adaptor signature or public key or T is invalid", async () => {
      ...
      const T = secp256k1.Point.BASE.multiply(t).toAffine();
      const T2 = secp256k1.Point.BASE.multiply(t2).toAffine();
  ...
  describe("extract", () => {
    it("should extract secret from two signatures correctly", async () => {
      ...
      const signatureScalar = (e + t) % secp256k1.Point.CURVE().n;
