...
describe("TypeCaster", () => {
  ...
  describe("array cast", () => {
    ...
    const bytes32Arrays = [[], addressArrays[1].map((e) => coder.encode(["address"], [e]))];
    const uint256Arrays = [[], bytes32Arrays[1].map((e) => <bigint>(<unknown>coder.decode(["uint256"], e)))];
    ...
    describe("asUint256Array", () => {
      it("should convert bytes32 array to uint256 array properly", async () => {
        for (const [idx, bytes32Array] of bytes32Arrays.entries()) {
          expect(await mock.asUint256ArrayFromBytes32Array(bytes32Array)).to.equal(uint256Arrays[idx]);
        }
      });

      it("should convert address array to uint256 array properly", async () => {
        for (const [idx, addressArray] of addressArrays.entries()) {
          expect(await mock.asUint256ArrayFromAddressArray(addressArray)).to.equal(uint256Arrays[idx]);
        }
      });
    });
    ...
    describe("asAddressArray", () => {
      it("should convert bytes32 array to address array properly", async () => {
        for (const [idx, bytes32Array] of bytes32Arrays.entries()) {
          expect(await mock.asAddressArrayFromBytes32Array(bytes32Array)).to.equal(addressArrays[idx]);
        }
      });

      it("should convert uint256 array to address array properly", async () => {
        for (const [idx, uint256Array] of uint256Arrays.entries()) {
          expect(await mock.asAddressArrayFromUint256Array(uint256Array)).to.equal(addressArrays[idx]);
        }
      });
    });
    ...
    describe("asBytes32Array", () => {
      it("should convert uint256 array to bytes32 array properly", async () => {
        for (const [idx, uint256Array] of uint256Arrays.entries()) {
          expect(await mock.asBytes32ArrayFromUint256Array(uint256Array)).to.equal(bytes32Arrays[idx]);
        }
      });

      it("should convert address array to bytes32 array properly", async () => {
        for (const [idx, addressArray] of addressArrays.entries()) {
          expect(await mock.asBytes32ArrayFromAddressArray(addressArray)).to.equal(bytes32Arrays[idx]);
        }
      });
    });
  });
  ...
  describe("singleton array", () => {
    ...
    it("should build singleton arrays properly", async () => {
      expect(await mock.asSingletonArrayFromUint256(123)).to.equal([123n]);

      expect(await mock.asSingletonArrayFromAddress(ETHER_ADDR)).to.equal([ETHER_ADDR]);
      expect(await mock.asSingletonArrayFromBool(false)).to.equal([false]);
      expect(await mock.asSingletonArrayFromString("1")).to.equal(["1"]);
      expect(await mock.asSingletonArrayFromBytes32(MOCKED_BYTES32)).to.equal([MOCKED_BYTES32]);
    });
  });
